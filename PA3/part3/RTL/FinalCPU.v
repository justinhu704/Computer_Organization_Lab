...
wire ext_we_out;
wire mem_sel_out;
wire [31:0] mem_out_input; // last input
...
always @(posedge clk or negedge rst_n) begin
    if (!rst_n)
        pc_reg <= 32'b0;
    else if (!test_normal_out & PC_write)
        pc_reg <= next_pc;
    else
        pc_reg <= pc_reg;
end
...
//========================================
wire [8:0] Control_sig, Control_sig_intput;
assign Control_sig = Control_stall ? 9'b0 : Control_sig_intput;

wire MemtoReg, Reg_w, MemRead, MemWrite, branch;
wire [1:0] ALU_Op;
wire ALUSrc, RegDst;

assign MemtoReg = Control_sig[8];
assign Reg_w = Control_sig[7];
assign MemRead = Control_sig[6];
assign MemWrite = Control_sig[5];
assign branch = Control_sig[4];
assign ALU_Op = Control_sig[3:2];
assign ALUSrc = Control_sig[1];
assign RegDst = Control_sig[0];

//Hazard Detection
reg IFID_mem_stall; // stall control
reg PC_write; // PC write control
reg Control_stall;

always@(*)begin
    if(IDEX_mem_M[2] & ((IDEX_mem_Rt_addr == IFID_mem_Instruction[25:21]) | (IDEX_mem_Rt_addr == IFID_mem_Instruction[20:16]))) begin
        IFID_mem_stall = 1'b0; // stall
        PC_write = 1'b0;
        Control_stall = 1'b1;
    end
    else begin
        IFID_mem_stall = 1'b1; // no stall
        PC_write = 1'b1;
        Control_stall = 1'b0;
    end
end
...
always@(*)begin
    case(ForwardA)
        2'b00: begin
            ALU_A = IDEX_mem_Rs_data;
        end
        2'b01: begin
            ALU_A = MEMWB_mem_WB[1] ? MEMWB_mem_DM_data : MEMWB_mem_ALU_out; // MemtoReg
        end
        2'b10: begin
            ALU_A = EXMEM_mem_ALU_out;
        end
    endcase 

    case(ForwardB)
        2'b00: begin
            ALU_B = IDEX_mem_Rt_data;
        end
        2'b01: begin
            ALU_B = MEMWB_mem_WB[1] ? MEMWB_mem_DM_data : MEMWB_mem_ALU_out; // MemtoReg
        end
        2'b10: begin
            ALU_B = EXMEM_mem_ALU_out;
        end
    endcase 
end
...
always@(*) begin
    ...
    if(EXMEM_mem_WB[0] &&
        ...
        (EXMEM_mem_RdRtaddr == IDEX_mem_Rs_addr)) begin
        ForwardA = 2'b10;
    end 
    // ForwardA: MEM hazard
    else if(MEMWB_mem_WB[0] && 
        (MEMWB_mem_RdRtaddr != 5'b0) && 
        !(EXMEM_mem_WB[0] &&
            (EXMEM_mem_RdRtaddr != 5'b0) &&
            (EXMEM_mem_RdRtaddr == IDEX_mem_Rs_addr)) &&
        (MEMWB_mem_RdRtaddr == IDEX_mem_Rs_addr)) begin
        ForwardA = 2'b01;
    end else begin
        ForwardA = 2'b00;
    end
    ...
    if (EXMEM_mem_WB[0] &&
        (EXMEM_mem_RdRtaddr != 5'b0) &&
        (EXMEM_mem_RdRtaddr == IDEX_mem_Rt_addr)) begin
        ForwardB = 2'b10;
    end 
    // ForwardB: MEM hazard
    else if (MEMWB_mem_WB[0] &&
            ...
            !(EXMEM_mem_WB[0] &&
                (EXMEM_mem_RdRtaddr != 5'b0) &&
                (EXMEM_mem_RdRtaddr == IDEX_mem_Rt_addr)) &&
            (MEMWB_mem_RdRtaddr == IDEX_mem_Rt_addr)) begin
        ForwardB = 2'b01;
    end else begin
        ForwardB = 2'b00;
    end
...
always@(posedge clk) begin
    if (!rst_n | test_normal_out)
        IFID_mem_Instruction <= 32'b0;
    else if (IFID_mem_stall)
...
reg [4:0] IDEX_mem_Rd_addr, IDEX_mem_Rt_addr, IDEX_mem_Rs_addr;
...
always@(posedge clk)begin
    if(!rst_n)begin
        ...
        IDEX_mem_Rs_addr <= IFID_mem_Instruction[25:21];
...
Control u_Control(
    ...
    .Control_sig(Control_sig_intput)
);

ALU u_ALU(
    .Rs_data(ALU_A),
    .Rt_data(IDEX_mem_EX[1] ? IDEX_mem_signext : ALU_B),
